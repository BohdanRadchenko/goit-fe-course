...
const getTotalBalance = users =>
    users.reduce((totalBalance, balance) => balance.balance + totalBalance, 0);
...
const getUniqueSkills = users =>
    users.reduce((iteral, skill) => [...iteral, ...skill.skills], [])
    .filter((skill, i, arr) => arr.indexOf(skill) === i).sort((a, b) => a > b ? 1 : -1);
// const reduced = users.reduce((iteral, skill) => [...iteral, ...skill.skills], []);
// const test = [...new Set(reduced)].sort();
// return test;
;

console.log(getUniqueSkills(users));
// console.log(['adipisicing', 'amet', 'anim', 'commodo', 'culpa', 'elit', 'ex', 'ipsum', 'irure', 'laborum', 'lorem', 'mollit', 'non', 'nostrud', 'nulla', 'proident', 'tempor', 'velit', 'veniam'].length);
